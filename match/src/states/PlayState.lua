    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    -- if no match was caused, revert tiles back to original positions
                    ...
                    if not(self.board:checkMatches()) then
                        -- if the resulting board after the last match does not contain any valid moves, reinitialize the board
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        -- add score & 1 sec per tile for each match
