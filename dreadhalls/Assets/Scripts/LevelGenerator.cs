...
public class LevelGenerator : MonoBehaviour {
	...
	// 4 possible holes in the level
	private int holeCount = 4;
	...
	void Start () {
		...
		// counter for how many holes we have
		int holes = 0;

		// increment player level for display
		level += 1;
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				// 90% chance to spawn a floor tile
				if (Random.Range(0, 10) < 1 && holes < holeCount && !mapData[z, x]) {
					// does not create a floor tile, increment # of holes
					holes++;
				} else {
