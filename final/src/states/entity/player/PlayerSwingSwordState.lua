...
function PlayerSwingSwordState:init(player, dungeon)
    ...
    self.player:changeAnimation('sword-' .. self.player.direction .. '-' .. self.player.meleeWeapon)
end
...
function PlayerSwingSwordState:update(dt)
    ...
    for k, entity in pairs(self.dungeon.currentRoom.entities) do
        if entity:collides(self.swordHitbox) then
            if math.random(1, 20) > entity.ac then
                entity:damage(1)

                -- additional potential on-hit effects
                if self.player.meleeWeapon == 'fire' then
                    if math.random(1, 2) == 1 then
                        entity:damage(1) -- bonus damage
                    end
                elseif self.player.meleeWeapon == 'poison' then
                    if math.random(1,2) == 1 then
                        table.insert(entity.conditions, 'poison')
                    end
                end
