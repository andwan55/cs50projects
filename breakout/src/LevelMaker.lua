...
function LevelMaker.createMap(level)
    ...
    -- if level is odd, add a locked brick
    local lockedBrick = (level % 2) == 1 and true or false
    
    -- determine in which column the locked brick will show up, if applicable
    local lockedPosX = math.random(1, numCols)
    local lockedPosY = math.random(1, numRows)
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            -- if we want locked bricks in this level and it's the brick's position, override all previous settings to make it locked
            if lockedBrick and x == lockedPosX and y == lockedPosY then
                b.locked = true
                b.tier = highestTier
