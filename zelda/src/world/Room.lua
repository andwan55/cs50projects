...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 then
            ...
            if entity.dead == false and entity.height == 16 and math.random(4) == 1 then
                ...
                heart.onCollide = function()
                    -- should add two HP to the player and disappear
            ...
            -- set entity to dead after to prevent repeated heart spawns
            entity.dead = true
    ...
    for k, object in pairs(self.objects) do
        ...
        if object.type == 'pot' and not object.solid and not object.thrown then
            object.x = self.player.x - (TILE_SIZE / 16)
            object.y = self.player.y - (TILE_SIZE / 2)
        end

        object:update(dt)
        
        -- if pot ends up out of bounds or travels more than 4 tiles distance after updating, immediately remove
        ...
        if object.type == 'pot' and object.thrown then
            ...
            object.distance >= (TILE_SIZE * 4) then
        ...
        if self.player:collides(object) then
            ...
            if object.type ~= nil then
                ...
                elseif object.type == 'pot' and object.solid then
                    ...
                    if love.keyboard.isDown('return') and not(self.player.holding) then
                        object.solid = false
        ...
        -- entity collision detection with a thrown pot
        for i = #self.entities, 1, -1 do
            local entity = self.entities[i]

            if object.type == 'pot' and object.thrown and entity:collides(object) then
                entity:damage(1)
                gSounds['hit-enemy']:play()
                object.state = 'broken'
                table.remove(self.objects, k)
